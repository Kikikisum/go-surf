...
from model.decode_up import NeRF360Decoder, PropMLP, NerfMLP
...
class SDFGridModel(torch.nn.Module):
    def __init__(self,
        ...
        # 计算世界坐标维度、体积原点、体素维度
        world_dims, volume_origin, voxel_dims = compute_world_dims(
            bounds,
            config["voxel_sizes"],
        ...
        self.decoder = NeRFDecoder(config["decoder"]["PropMLP"],
                                   config["decoder"]["NerfMLP"],
    ...
    def forward(self, rays_o, rays_d, target_rgb_select, target_depth_select, inv_s=None, smoothness_std=0., iter=0):
        # 提案网络阶段
        prop_rend_dict = render_rays(
            self.sdf_decoder,  # 使用 PropMLP 进行渲染
            self.sdf_decoder,
            self.grid,
            self.volume_origin,
            self.world_dims,
            ...
            use_view_dirs=self.config["decoder"]["PropMLP"]["use_view_dirs"],
            use_normals=self.config["decoder"]["PropMLP"]["use_normals"],
            concat_qp_to_rgb=self.config["decoder"]["PropMLP"]["concat_qp"],
            concat_qp_to_sdf=self.config["decoder"]["PropMLP"]["concat_qp"],
            concat_dot_prod_to_rgb=self.config["decoder"]["PropMLP"]["use_dot_prod"],
            rgb_feature_dim=self.config["rgb_feature_dim"],
            iter=iter
        )

        # 使用 NerfMLP 进行详细渲染
        nerf_rend_dict = render_rays(
            self.rgb_decoder,  # 使用 NerfMLP 进行渲染
            self.rgb_decoder,
            self.grid,
            self.volume_origin,
            self.world_dims,
            ...
            inv_s=inv_s,
            smoothness_std=smoothness_std,
            use_view_dirs=self.config["decoder"]["NerfMLP"]["use_view_dirs"],
            use_normals=self.config["decoder"]["NerfMLP"]["use_normals"],
            concat_qp_to_rgb=self.config["decoder"]["NerfMLP"]["concat_qp"],
            concat_qp_to_sdf=self.config["decoder"]["NerfMLP"]["concat_qp"],
            concat_dot_prod_to_rgb=self.config["decoder"]["NerfMLP"]["use_dot_prod"],
            rgb_feature_dim=self.config["rgb_feature_dim"],
            iter=iter
        )

        rendered_rgb, rendered_depth = nerf_rend_dict["rgb"], nerf_rend_dict["depth"]
        ...
        ret = {
            ...
            "sdf_loss": nerf_rend_dict["sdf_loss"],
            "fs_loss": nerf_rend_dict["fs_loss"],
            "normal_regularisation_loss": nerf_rend_dict["normal_regularisation_loss"],
            "normal_supervision_loss": nerf_rend_dict["normal_supervision_loss"],
            "eikonal_loss": nerf_rend_dict["eikonal_loss"],
            "psnr": psnr
        }
