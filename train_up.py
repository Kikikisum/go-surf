...
from model.sdf_grid_model_up import SDFGridModel, qp_to_sdf
...
def main(args):
    ...
    if config["dataset_type"] == "scannet":
        dataset = ScannetDataset(os.path.join(config["datasets_dir"], args.scene), trainskip=config["trainskip"],
    ...
    if args.start_iter > 0:
        state = torch.load(os.path.join(config["checkpoints_dir"], "chkpt_{}".format(args.start_iter)),
                           map_location=device)
        inv_s = state["inv_s"]
