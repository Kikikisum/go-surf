...
from model.sdf_grid_model_up import SDFGridModel, compute_grads
...
def main(args):
    ...
    if config["dataset_type"] == "scannet":
        dataset = ScannetDataset(os.path.join(config["datasets_dir"], args.scene), load=False,
                                 trainskip=config["trainskip"], device=torch.device("cpu"))
    elif config["dataset_type"] == "rgbd":
        dataset = RGBDDataset(os.path.join(config["datasets_dir"], args.scene), load=False,
                              trainskip=config["trainskip"], device=torch.device("cpu"))
    else:
